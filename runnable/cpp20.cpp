...
public:
    ...
    struct promise_type {
        ...
        std::suspend_always yield_value(T value) { state = std::move(value); return {}; }
    ...
    std::optional<T> next() {
        if (!handle) return std::optional<T>{};
        handle.resume();
        auto state = handle.promise().state;
        if (handle.done()) state.reset();
        return state;
    }
...
concept incrementable = requires (T x) {
    ...
    {x.increment()} -> std::same_as<typename T::ValueType>;
};

void test_concepts_exprs() {
    auto lambda = [](incrementable auto v) { return v.increment(); };
    ObjA a{0};
    int result = lambda(a);
    ASSERT_EQ(result, 1);
}
...
int set_zero_then_sum(std::span<int> span) {
    if (!span.empty())
        span[0] = 0;
    int sum = 0;
    std::for_each(span.begin(), span.end(), [&](auto& v) { sum += v; });
    return sum;
}
