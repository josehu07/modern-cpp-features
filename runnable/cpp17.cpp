...
void test_structured_bindings() {
    ...
    auto func = [](int i, double j) -> decltype(auto) { return std::make_tuple(i, j); };
    auto [i, j] = func(1, 2.3);
    ASSERT_EQ(i, 1);
    ASSERT_EQ(j, 2.3);
...
void test_switch_initializer() {
    auto should_throw = []() {
        ...
        switch (ObjB test_b(false); auto s = test_b.status()) {
            case ObjB::OK:     test_b.do_work();                              break;
            case ObjB::FAILED: throw std::runtime_error(ObjB::status_msg(s)); break;
            default: break;
        }
    };
...
void test_std_invoke() {
    ...
    auto add_func = [](int x, double y) { return x + y; };
