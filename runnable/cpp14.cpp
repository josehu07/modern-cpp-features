...
{
    int j = 1;
    auto generator = [x = times_10(j)]() mutable
    { return x++; };
    j = 2;
...
{
    auto identity_ref_lambda = [](auto &x) -> auto &
    { return identity_ref(x); };
    int x = 123;
...
void test_decltype_auto()
{
    ...
    int z = 123;
    static_assert(std::is_same<int, decltype(identity_auto(z))>::value, "decltype not working");
    static_assert(std::is_same<const int &, decltype(identity_decltype_auto(z))>::value, "decltype not working");
}
...
[[deprecated("this function is deprecated")]] int legacy_func() { return 7; }
